...
docker run --restart=on-failure --env-file=.env.${BOT_NAME} -v $(pwd)/stats.json:/app/stats.json -v $(pwd)/previous_image_themes.txt:/app/previous_image_themes.txt ${BOT_NAME}
