...
- **`add_occasion.py`** — add/list/delete "on this day" chat-image occasions (date-keyed prompt directives). Opens `$EDITOR` on a pre-filled template, or runs non-interactively if `--server-id`/`--date`/`--directive` (or `--global`) are all supplied. See the README's "On this day image occasions" section.
